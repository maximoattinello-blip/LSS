...
def migrate_db():
	...
	# Crear tabla special_days si no existe (migración segura)
...
def api_reserve():
	...
	if is_free and redemption_token:
		...
		# Horas bajas: multiplicador x2.5
		if (8 <= hour < 11) or (13 <= hour < 16) or hour >= 22:
			multiplier *= 2.5

		# Días especiales configurados por admins: multiplicador adicional
		month_day_check = start_dt.strftime('%m-%d')
		year_check      = start_dt.strftime('%Y-%m-%d')
		special = conn.execute('''
			SELECT multiplier FROM special_days
			WHERE (recurrence='ONCE' AND date=?)
			   OR (recurrence='YEARLY' AND substr(date,6)=?)
			ORDER BY multiplier DESC
			LIMIT 1
		''', (year_check, month_day_check)).fetchone()
		if special:
...
@app.route('/api/special-days')
def api_special_days():
    """Devuelve los días especiales (público, para mostrar en calendarios)."""
    conn = get_db()
    rows = conn.execute('SELECT * FROM special_days ORDER BY date ASC').fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])

@app.route('/api/admin/special-days', methods=['POST'])
@admin_required
def api_add_special_day():
    ...
    if not name or not date_val:
        return jsonify({'success': False, 'error': 'Faltan campos'}), 400
    if recurrence not in ('ONCE', 'YEARLY'):
        recurrence = 'ONCE'
    conn = get_db()
    conn.execute(
        'INSERT INTO special_days (name, date, recurrence, multiplier, created_by) VALUES (?,?,?,?,?)',
        (date_val, name, recurrence, multiplier, session['user_id'])
    )
    conn.commit()
    conn.close()
    return jsonify({'success': True})

@app.route('/api/admin/special-days/<int:day_id>', methods=['DELETE'])
@admin_required
...
@login_required
def api_points():
    """Endpoint liviano para polling de puntos en tiempo real."""
    conn = get_db()
